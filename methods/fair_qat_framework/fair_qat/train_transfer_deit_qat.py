"""
Unified transfer-learning DeiT QAT entry for AAAI experiments.

Supports 5 datasets (aircraft, food101, pets, flowers102, cars) with
DeiT-Tiny backbone. FP32 from ImageNet pretrained, QAT from FP32 fine-tuned.
"""
...
from data_transfer import build_transfer_loaders
...
def main():
    ...
    if not dataset_name or num_classes <= 0:
        raise ValueError(f"Invalid dataset_name={dataset_name} num_classes={num_classes}")
    ...
    if method in {"qvit", "qvit_nodistill", "gplq"} and bits == 1:
    ...
    if method == "fp32":
        ...
        if not ckpt or not os.path.isfile(ckpt):
            raise FileNotFoundError(
                f"FP32 fine-tune requires existing pretrained_checkpoint: {ckpt}"
            )
        if is_main_process(rank):
            print(f"[CHECKPOINT] Loading ImageNet pretrained from {ckpt}")
    else:
        ckpt = config.get("initial_checkpoint", "")
        ckpt_type = config.get("initial_checkpoint_type", "")
        if ckpt_type != "transfer_fp32_finetuned":
            raise ValueError(
                "QAT must start from transfer FP32 fine-tuned checkpoint: "
        ...
        if is_main_process(rank):
            print(f"[CHECKPOINT] Loading transfer FP32 checkpoint from {ckpt}")
    ...
    train_loader, val_loader = build_transfer_loaders(run_config)
    if is_main_process(rank):
        print(
            f"[DATA] per_gpu_batch={per_gpu_batch} global_batch={global_batch} "
