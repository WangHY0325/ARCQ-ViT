"""
Unified CIFAR-100 DeiT QAT entry for AAAI transfer experiments.

This entry uses timm DeiT backbones (Tiny/Small/Base) instead of the older
fixed Tiny-like FairLowBitVisionTransformer CIFAR script.
"""
...
from fair_qat.trace_utils import TraceCollector
from fair_qat.aoq_lsq_backend import (
    set_aoq_epoch,
    reset_aoq_aux_loss,
    get_aoq_aux_loss,
)
...
def should_save_checkpoint(method: str, config: dict) -> bool:
    """Keep FP32 and our DDFZ-family checkpoints by default."""
    if "save_checkpoints" in config:
        return bool(config["save_checkpoints"])
    method = str(method).lower()
    return "ddfz" in method
...
def train_one_epoch(
    ...
    for step, (images, targets) in enumerate(loader, start=1):
        ...
        if is_aoq_lsq:
            reset_aoq_aux_loss(images.device)
        ...
        with amp_context(device):
            ...
            if is_aoq_lsq:
                aoq_aux = get_aoq_aux_loss()
                if aoq_aux is not None and epoch > aoq_aux_start_epoch:
                    loss = loss + aoq_aux_lambda * aoq_aux / grad_accum_steps
                    aoq_aux_loss_sum += float(aoq_aux.detach())
        ...
        if rank == 0 and log_interval > 0 and (step == 1 or step % log_interval == 0):
            ...
            img_s = seen / elapsed
            aoq_str = f" aoq_aux={aoq_aux_loss_sum:.6f}" if is_aoq_lsq else ""
            print(
                f"[TRAIN_STEP] epoch={epoch} step={step}/{len(loader)} "
                f"loss={last_loss:.4f} img_s={img_s:.1f}{aoq_str}",
                flush=True,
            )
...
@torch.no_grad()
def fit_or_load_pca_projector(teacher_model, teacher_hook, train_loader, device, config):
...
class OfficialPackQViTLoss(nn.Module):
    """PackQViT official DeiT-style distillation loss.

    Official PackQViT uses the DeiT DistillationLoss: base CE on the main
    student logits plus hard/soft distillation from a frozen teacher. If the
    student returns a tuple, the second tensor is used as the distillation
    branch, matching the official implementation. The fair backend returns a
    single-logit DeiT model, so that tensor is used for both branches.
    """
    ...
    def forward(self, images, outputs, targets):
        ...
        if self.distillation_type == "soft":
            ...
            raise ValueError("PackQViT distillation_type must be hard or soft, got " + str(self.distillation_type))
...
def build_kd_criterion(config, model, train_loader, base_criterion, device):
    ...
    if method == "packqvit" or distillation_mode == "official_packqvit":
        ...
        print(
            "[PACKQVIT] official_distillation=" + distillation_type
            + " alpha=" + str(alpha)
            + " tau=" + str(tau)
            + " teacher=" + str(config.get("teacher") or config.get("teacher_checkpoint")),
            flush=True,
        )
        from fair_qat.official_kd_adapters import build_official_packqvit_distillation_loss
        return build_official_packqvit_distillation_loss(
            base_criterion, teacher, distillation_type, alpha, tau
        )
    if method == "n2uq" or distillation_mode == "official_n2uq":
        teacher = build_fp32_teacher(config, device)
        print(
            "[N2UQ] official_distribution_loss=True teacher="
            + str(config.get("teacher") or config.get("teacher_checkpoint")),
            flush=True,
        )
        from fair_qat.official_kd_adapters import build_official_n2uq_distribution_loss
        return build_official_n2uq_distribution_loss(teacher)
    ...
    if kd_type == "gplq_tcs":
        ...
        pca_projector = fit_or_load_pca_projector(teacher, teacher_hook, train_loader, device, config)
        print("[KD] type=gplq_tcs alpha=" + str(alpha) + " pca_dim=" + str(pca_projector.components.shape[0]), flush=True)
...
def main():
    ...
    if str(config.get("dataset", "")).lower() != "cifar100":
        raise ValueError(f"train_cifar100_deit_qat.py only supports dataset=cifar100, got {config.get('dataset')}")
    ...
    bits = int(config.get("w_bits", config.get("bits", 32)))
    if is_main_process(rank) and str(method).lower() == "n2uq":
        print("[N2UQ] learnable_threshold=True output_levels=uniform", flush=True)
    if method in {"qvit", "gplq"} and bits == 1:
        raise ValueError(f"{method} W1A1 is intentionally unsupported in this matrix.")
    if method == "fp32":
        ckpt = config.get("pretrained_checkpoint", "")
        if not ckpt or not os.path.isfile(ckpt):
            raise FileNotFoundError(f"FP32 fine-tune requires existing ImageNet pretrained_checkpoint: {ckpt}")
    else:
        ckpt = config.get("initial_checkpoint", "")
        ckpt_type = config.get("initial_checkpoint_type", "")
        if ckpt_type != "cifar100_fp32_finetuned":
            raise ValueError(
                "QAT must start from CIFAR-100 FP32 checkpoint: "
                f"initial_checkpoint_type={ckpt_type}"
            )
        if not ckpt or not os.path.isfile(ckpt):
            raise FileNotFoundError(
                "QAT initial_checkpoint is missing. Do not fall back to ImageNet pretrained: "
                f"{ckpt}"
            )
    ...
    train_loader, val_loader = build_cifar100_loaders(run_config)
    if is_main_process(rank):
        print(
    ...
    is_nulsq_wa = str(method).lower() in {"nulsq_wa", "nulsq"}
    if is_nulsq_wa:
        from fair_qat.nulsq_backend import initialize_nulsq_model
        initialize_nulsq_model(model, train_loader, device, distributed, rank)
    ...
    # ---- Trace collector (Figure 4) ----
    if is_main_process(rank) and bool(config.get("trace_enabled", False)):
        trace_collector = TraceCollector(model, config)
        # Parse compile epochs for compile_index tracking
        pc_ce = config.get("pc_compile_epochs", [])
        if isinstance(pc_ce, str):
            pc_ce = [float(x.strip()) for x in pc_ce.split(",") if x.strip()]
        pc_compile_epochs = sorted({int(e) for e in pc_ce})
        _compile_idx = 0
    else:
    ...
    is_aoq_lsq = str(config.get("method", "")).lower() == "aoq_lsq"
    if is_nulsq_wa:
        from fair_qat.nulsq_backend import nulsq_parameter_groups
        optimizer = optim.AdamW(
            nulsq_parameter_groups(
                model,
                lr=float(config.get("lr", 5e-5)),
                weight_decay=float(config.get("weight_decay", 0.05)),
                coeff_qparm_lr=float(config.get("nulsq_coeff_qparm_lr", 0.01)),
                qparm_wd=float(config.get("nulsq_qparm_wd", 1e-4)),
            ),
            betas=(0.9, 0.999),
        )
    elif is_aoq_lsq:
        # AOQ: separate quantizer params (no decay, 0.1x lr)
        decay_params = []
        no_decay_params = []
        quant_params = []
        lr = float(config.get("lr", 5e-5))
        wd = float(config.get("weight_decay", 0.05))
        for name, p in model.named_parameters():
            if not p.requires_grad:
                continue
            if "quantizer" in name or "step_size" in name or "init_interval" in name or "alpha" in name:
    ...
    if early_stop_enabled:
        early_stop_start_epoch = int(config.get("early_stop_start_epoch", 30))
        patience = int(config.get("early_stop_patience", 30))
        min_delta = float(config.get("early_stop_min_delta", 0.01))
    else:
        early_stop_start_epoch = epochs + 1
        patience = 0
        min_delta = 0.0
    best_top1 = -1.0
    best_epoch = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        if trace_collector is not None:
            trace_collector.reset_epoch(epoch)
        t0 = time.time()
        train_stats = train_one_epoch(
            ...
            is_aoq_lsq=is_aoq_lsq,
            aoq_aux_lambda=float(config.get("aoq_aux_lambda", 0.01)),
            aoq_aux_start_epoch=int(config.get("aoq_aux_start_epoch", 150)),
        )
        val_stats = validate(model, val_loader, criterion, device, distributed)
        scheduler.step()
        ...
        # ---- Trace (Figure 4) ----
        if trace_collector is not None:
            compile_index = sum(1 for ce in pc_compile_epochs if ce <= epoch)
            trace_collector.record_train_val(
                epoch=epoch,
            ...
            trace_collector.on_epoch_end(epoch, compile_index)
            trace_collector.flush()
        # ----

        if early_stop_enabled and epoch >= early_stop_start_epoch and stale_epochs >= patience:
            if is_main_process(rank):
